...
**你能观察到的效果**：Subagent 不会因为"不知道项目用什么"而引入别的框架或库，也不会去改写集外的文件。
...
当前 Feature 是哪个、写集包含哪些文件、AC 有哪些、下一项是什么。

## 为什么 Subagent 需要这个

### 委派子任务时

主 Agent 把前端页面交给 Subagent 做。Subagent 拿到上下文后知道项目是 Vue 3 + Element Plus，只能改写集内的页面文件，不会顺手改别处。
...
`Agent` 工具的 `fork` 类型会继承主 Agent 的完整对话历史；不指定类型的普通 Subagent 不继承。

<Aside type="caution">
普通 Subagent 没有主 Agent 的对话历史，项目上下文完全依赖这个 Hook。Hook 没生效时它就是在裸跑。
</Aside>
...
# 自定义开发者 ID（仅在 Git identity 不可用时生效）
